...
function getValueRanges($min = 0, $max = 100, $step = 5)
{
    $start = 0;
    $valueRange = [];
    while ($min < $max && $min != $max) {
        $start = (($start + $step) > $max) ? $max : $start + $step;
        array_push($valueRange, ["name" => "$min-$start", "value" => "$min-$start"]);
        $min = $start;
    }
    array_push($valueRange, ["name" => "Other pr", "value" => "other_preis"]);
    return $valueRange;
}
 ...
 * @param int $user_id
 * @return int
 * @author Tanima Yousuf
 */
// function getCompanyIdByAdmin($user_id){
//     if(auth()->user()->type == 'company_admin'){
//         $admin = CompanyAdmin::where('user_id', $user_id)->firstOrFail();
//     }else if(auth()->user()->type == 'company_sub_admin'){
//         $admin = CompanySubAdmin::where('user_id', $user_id)->firstOrFail();
//     }
...
function getCountryNameFromCode($code, $local = 'en')
{
    ...
    if (isset($countries[$local]) && isset($countries[$local][$countryCode])) {
