<template>
    <div>
        <div class="container-box">
            ...
            />
            <div class="page-title">{{ $t('Edit Product Document') }}</div>
            <div class="flex mt-6 items-center">
                <div class="w-1/5 items-start flex flex-col">
                    <label class="form-label block mb-[8px] text-base"
                        >{{ $t('Product ID') }}</label
                    >
                    <div class="text-input" id="product-id">{{product.prefix_id}}</div>
                </div>
                <div class="w-1/5 mt-2 relative">
                    <TextInput
                        style="width:253px"
                        id="serial_number"
                        :label="`${$t('Serial Nr.')}*`"
            ...
            <div class="flex mt-6 items-center">
                ...
                <div class="w-1/5 mt-2">
                    <div class="form-label mb-4 text-[#5D5D60]">{{ $t('PDF Access') }}</div>
                    <CheckBoxInput
            ...
            <div class="mt-6">
                ...
                <div class="mt-6">
                    ...
                    <div>
                        <Button class="mt-6 flex !justify-start p-4 text-black" style="width:253px" @click="incrementFile('modification_guide')"
                        id="modification_guide-add-btn"
                            ><PlusIcon class="mr-12" /><span
                                >{{ $t('Add more') }}</span
                            ></Button
                        >
...
const props = defineProps(['product', 'suppliers', 'categories'])
const form = reactive({
    id: props.product.id,
    prefix_id: props.product.prefix_id,
    serial_number: props.product.serial_number,
    warranty_start_date: props.product.warranty_start_date,
    warranty_end_date: props.product.warranty_end_date,
    name: props.product.name,
    supplier_id: props.product.supplier_id,
    category_id: props.product.category_id,
    public_access: props.product.public_access,
    technical: [
        {
            doc_id: "",
            name: "",
...
const fileUpload = (e) => {
    form.isDirty=true
    form[e.type][e.index].name = form[e.type][e.index].name;
    form[e.type][e.index].doc_type = form[e.type][e.index].doc_type;
...
const fileRemove = (e) => {
    ...
    form[e.type][e.index]={doc_id:"", name:"", doc_type: "", file:""};
    form[e.type+"_count"]--
...
const submitForm = () => {
    console.log(form.technical);
    if(isValid() == false){
        return;
    }
    Inertia
        .put(route("products.update",{ product: props.product.id }), form, {
            onError: (err) => {
...
props.product.documents.forEach(function(item, index){
    if(form[item.section][0].file==""){
        form[item.section].splice(0,1)
    }
    form[item.section].push({
        doc_id: item.id,
        name: item.name,
        doc_type: item.type,
        file: item.filepath,
