<template>
    <div class="page bg-white p-8 rounded-[8px]">
        <div class="head mb-12">
            ...
            />
            <h1 class="title font-bold text-20 text-[#7059E2]">{{ $t('Edit Admin User') }}</h1>
        </div>
                ...
                <div class="form__section">
                    <div class="wrapper flex gap-[30px]">
                        <div class="input-field flex gap-3 flex-col w-[285px]">
                            ...
                            <p class="p-0 text-[#5D5D60]">{{ company_admin.prefix_id }}</p>
                        </div>
...
onMounted(()=> {
    const {first_name, last_name, email, company_id, language_id} = props.company_admin
...
const submitForm = () => {
    form.put(route('company-admins.update', {company_admin: props.company_admin.id}), {
