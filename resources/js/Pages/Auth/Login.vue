    ...
    <div class="login w-full h-screen flex">
        <div class="form__section flex justify-center w-1/2 bg-[#ECF6FD] pt-10 pb-16">
            <div class="form block w-[305px] relative">
                ...
                <h3 class="text-20 text-[#5D5D60] font-bold mb-5">{{ $t('LogIn') }}</h3>
                <form @submit.prevent="login">
                    <div class="form-group mb-7">
                        <label
                            class="text-16 font-bold  text-[#7059E2]"
                            for="email"
                            >{{ $t('Email') }}*</label
                        >
                        <TextInput
                            ...
                            v-model="form.email"
                            class="shadow-form"
                            :class="{'border-none': !form.errors.invalid_credentials}"
                            :error="form.errors.invalid_credentials"
                            :show_error="false"
                        />
                    </div>
                    <div class="form-group mb-7">
                    ...
                    <div class="form-group">
                        <VueHcaptcha
                            sitekey="62624483-c2b8-414e-8705-2fb945a98474"
                            @verify="verifyCaptcha"
                            @error="verifyCaptchaFail"
                            id="captcha"
                            language="en"
                            :key="form.key"
                        ></VueHcaptcha>
                    </div>
                    <div class="form-group mt-[32px]">
                        <button
                            class="btn-block font-bold text-18 border-none rounded-lg h-12 text-white bg-primary-1-1"
                            type="submit"
                            :class="{disabled: form.processing || !form.captcha_verify}"
                            :disabled="form.processing || !form.captcha_verify"
                        >
                            {{ $t('Sign in') }}
                        </button>
                        <span class="mt-3 text-12 text-[#FF0000]" v-if="form.errors.invalid_credentials">{{ $t('Email :slash password is not correct', {slash: '/'}) }}</span>
                    </div>

                    <div class="form-group absolute bottom-0 forgot-password  text-center">
                        <Link
                            :href="route('password.request')"
                            class="font-16 text-[#5D5D60] underline"
                        >
                        {{ $t('Forgot Password?') }}
                        </Link>
                    </div>
        ...
        <div class="background__section w-1/2 relative">
            <img src="/images/left_background.png" class="max-h-screen w-full" alt="">
            <div class="language absolute right-10 bottom-10">
...
<script >
    ...
    import { computed, inject, onMounted, useAttrs } from "@vue/runtime-core";
    ...
    export default {
        ...
        setup(){
            ...
            const verifyCaptcha = (token, key) =>{
                form.captcha_verify = true;
            }
            const verifyCaptchaFail = (err) =>{
                console.log(err);
            }
            ...
            form.langCode = usePage().props.value.language_code ?? (cacheLanguage ?? lang) ;
