        ...
        <div class="contnet min-h-screen bg-background rounded-t-[32px] md:rounded-[0px] flex flex-col">
            <div class="card flex flex-col justify-center md:px-[163px] px-4 py-9 items-center">
                ...
                <form
                    ...
                    class="form flex flex-col w-[300px]"
                >
                    <h4 class="text-35 text-base text-20 font-bold">
                        {{ $t('Forgot Password?') }}
                    </h4>
                    <p class="mt-[24px] text-base text-[14px] leading-[17px]">
                        {{
                            $t("No problem! Please enter your registered email address and we will send you instructions on how to reset your password.")
                        }}
                    </p>
                    <div class="mt-[34px]">
                        <TextInput
                            :label="$t(`E-Mail`)"
                            :placeholder="$t('eg. john.doe@mail.com')"
                            v-model="form.email"
                            :error="form.errors.email"
                            inputClass="border-[0px] shadow-form"
                            labelClass="text-[#7059E2]"
                            errorClass="!text-[14px] leading-[17px]"
                    ...
                    <div class="mt-[32px]">
                        <PrimaryButton
                            class="text-white font-bold text-18 px-[21px] py-[18px] bg-[#7059E2] text-center rounded-[8px] flex justify-center w-full"
                            :disabled="v$.$invalid || !form.captcha_verify || form.processing"
                        >{{ $t('Send Instruction') }}</PrimaryButton>
                    </div>
                </form>
            </div>
            <Link
