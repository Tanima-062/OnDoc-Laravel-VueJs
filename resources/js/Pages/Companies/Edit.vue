<template>
    <div class="page bg-white p-8 rounded-[8px]">
        <div class="head mb-12">
            ...
            />
            <h1 class="title font-bold text-20 text-[#7059E2]">{{ $t('Edit Company') }}</h1>
        </div>
                ...
                <div class="form__section">
                    <div class="wrapper flex gap-[30px]">
                        <div class="input-field flex gap-3 flex-col w-[285px]">
                            ...
                            <p class="p-0 text-[#5D5D60]">{{ company.prefix_id }}</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="input-field w-[285px]">
                            <div class="flex">
                                ...
                                <div>
                                    ...
                                    <p
                                        v-if="!form.logo && company.logo && form.company_image_url"
                                            class="flex items-center clear-image form-label text-16 text-red-500 font-bold gap-2.5 cursor-pointer mt-5"
                                            @click="removeImage"
                                        >
                                        <CloseIcon fillColor="#f24a44" height="12" width="12" />
                                            {{ $t("Remove Image") }}
                                    </p>
                                    <input
                                        ref="companyFile"
...
const removeImage = ()=> {
        form.company_image_url = null;
        axios.delete(route('companies.remove.logo', {company:props.company.id}))
            .then(res=> {
                console.log(res)
            })
            .catch(err=> {
...
onMounted(()=> {
    const {name,street,house_number, postal_code,city,country_code, contact_persion_first_name, contact_persion_last_name, contact_persion_email, phone_number, country_iso_code } = props.company
    ...
    form.contact_persion_first_name  = contact_persion_first_name
    form.contact_persion_last_name  = contact_persion_last_name
    form.contact_persion_email  = contact_persion_email
    form.phone_number  = phone_number
    form.country_iso_code  = country_iso_code
    form.company_image_url = props.company.logo_url
})
